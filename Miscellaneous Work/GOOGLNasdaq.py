...
table=sqlContext.sql("select Floor(Date) as Date, Exchange, Ticker, TradeVol from USSEC where Date Like '2%' and Exchange = 'Nasdaq' and Ticker = 'GOOGL' order by Date")
...
table.rdd.map(lambda x: ",".join(map(str, x))).coalesce(1).saveAsTextFile("/user/cloudera/output_spark/GOOGLNasdaq.csv")
